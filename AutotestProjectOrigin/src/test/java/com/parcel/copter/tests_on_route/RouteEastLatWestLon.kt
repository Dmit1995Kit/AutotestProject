...
//Восток-запад
class RouteEastLatWestLon:TestCase() {
    private val deltaAsimut = 4

    private val eastlatwestlon = EastLatWestLon()
    private val eastlatwestlon_1 = EastLatWestLon_1()
    val error = readLine()
    ...
    fun testValueAsimutsNorthEast() {
        arrayRoute.add(eastlatwestlon)
//        arrayRoute.add(eastlatwestlon_1)

        for (i_1 in 0 until arrayRoute.size) {
            if (i_1 == 0)
                println("- - - - -  Сoordinates for East latitude and West longitude - - - - - ")
    ...
    fun classSwitch(maker: Any): CreatingClasses_6 {
        if (maker.equals(eastlatwestlon))
            return SwitchingEastLatWestLon()
        else if(maker.equals(eastlatwestlon_1))
            return SwitchingEastLatWestLon_1()

        throw RuntimeException("Error $maker")
    }
}
class EastLatWestLon : CoordinateSystem_6 {
    //маршрут по близкому расстоянию (тип маршрута линейный)
    private val route = Route()
    private val asimuts = arrayListOf<Double>()
    init {

        route.points.add(MapPoint(54.42203451633279,-105.78728759765586, 0, 0))
        route.points.add(MapPoint(54.42655639030245,-107.13621826171843, 0, 1))
        route.points.add(MapPoint(54.4242542293525,-108.16017837524349, 0, 2))
        route.points.add(MapPoint(54.422452447614845,-108.84030075073156, 0, 3))
...
class EastLatWestLon_1 : CoordinateSystem_6 {
...
interface CoordinateSystem_6 {
    fun createRouts(): Route
    fun createAsimuts(): ArrayList<Double>
}
interface CreatingClasses_6 {
    fun createCoordinates(): CoordinateSystem_6
}

class SwitchingEastLatWestLon : CreatingClasses_6 {
    override fun createCoordinates(): CoordinateSystem_6 {
        return EastLatWestLon()
    }
}
class SwitchingEastLatWestLon_1 : CreatingClasses_6 {
    override fun createCoordinates(): CoordinateSystem_6 {
        return EastLatWestLon_1()
    }
}